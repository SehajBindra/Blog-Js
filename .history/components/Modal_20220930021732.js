...
const people = [
  { id: 1, name: "Technology", unavailable: false },
  { id: 2, name: "Sports", unavailable: false },
  { id: 3, name: "Art & Entertainment", unavailable: false },
  { id: 4, name: "Science", unavailable: true },
  { id: 5, name: "others", unavailable: false },
];
            ...
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:py-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6">
              <div>
                ...
                >
                  {({ open }) => (
                    <>
                      <Listbox.Button className="text-sm text-center flex flex-col justify-center align-middle mx-auto ">
                        {selectedPeople.map((person) => person.name).join(", ")}
                        <ChevronUpDownIcon
                          className="h-5 flex items-center w-5 text-gray-400"
                          aria-hidden="true"
                        />
                      </Listbox.Button>
                      <Transition
                        ...
                        <Listbox.Options>
                          {people.map((person) => (
                            <Listbox.Option
                              key={person.id}
                              value={person}
                              className="active:bg-gray-100 transition-all text-sm text-center flex flex-col justify-center align-middle mx-auto duration-200 active:text-black active:rounded-md bg-white text-black"
                            >
