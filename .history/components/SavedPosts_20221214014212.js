...
function SavedPosts({ product }) {
  ...
  useEffect(() => {
    ...
    onSnapshot(
      collection(db, "post", "saved", session?.user?.name),
      (snapshot) => setSave(snapshot.docs)
    );
  }, [db]);
  ...
  const savePost = async () => {
    const id = `${product._id}`;
    if (hasSaved) {
      await deleteDoc(doc(db, "post", id, "saved", session?.user.name));
    } else {
      await setDoc(doc(db, "post", id, "saved", session?.user.name), {
