...
function Post({ product }) {
  ...
  return (
    <div className=" my-8    text-white">
      ...
      <div className="flex items-center mr-12 p-5  overscroll-y-auto ">
      ...
      {updateMode ? (
        <>
          ...
          {/* <div className="text-white">
            <ReactQuill
              ...
              className=" border-none  placeholder-shown:text-white"
              theme="snow"
              placeholder="lets edit!!"
              value={desc}
              onChange={setDesc}
            />
          </div> */}
          <div style={{ width: 500, height: 200 }}>
            <div value={desc} onChange={setDesc} ref={quillRef}></div>
          </div>
        </>
      ) : (
        <p className="text-base break-words my-4 max-w-2xl ">
          {" "}
          {product.desc ||
            desc.map((ops) => <h1 key={ops.insert}>{ops[0].insert}</h1>)}{" "}
        </p>
      )}
