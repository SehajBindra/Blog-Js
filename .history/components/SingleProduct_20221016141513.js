...
function Post({ product }) {
  ...
  return (
    <div className=" my-7  overflow-x-hidden  rounded-sm">
      {/* header */}
          ...
          <Menu as="div">
            {({ open }) => (
              <>
                ...
                >
                  {open && (
                    <div className="w-56 mx-auto fixed border rounded-lg">
                      {/*
                Using the `static` prop, the `Menu.Items` are always
                rendered and the `open` state is ignored.
              */}
                      <Menu.Items
                        className="flex flex-col flex-grow-0 w-32 sm:w-60 py-2 px-4  my-1 bg-white shadow-lg rounded-md  "
                        as="div"
                      >
                        <Menu.Item>
                          {({ active }) => (
