...
function SavedPosts({ product }) {
  ...
  useEffect(() => {
    ...
    onSnapshot(collection(db, "saved", id, session?.user.name), (snapshot) =>
  ...
  const savePost = async () => {
    const id = `${product._id}`;
    if (hasSaved) {
      await deleteDoc(doc(db, id, "saved", session?.user?.name, "userSaved"));

      toast.error("Remove from saved");
    } else {
      const docRef = await setDoc(
        doc(db, id, "saved", session?.user?.name, "userSaved"),
        {
