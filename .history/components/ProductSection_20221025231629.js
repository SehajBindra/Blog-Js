...
function ProductSection({ product }) {
  ...
  return (
    ...
    >
      <div className=" my-4  flex-shrink-0 max-w-sm   sm:my-8">
        <Link href={`/product/${product._id}`}>
          <img
            className="rounded-md h-[10rem]  max-w-xs  w-[10rem] hover:opacity-80 hover:shadow-lg  transition duration-200 ease-out  cursor-pointer object-cover flex-shrink-0"
            src={product.img}
          />
        </Link>
      </div>
