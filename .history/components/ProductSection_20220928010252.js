...
function ProductSection({ product }) {
  ...
  return (
    <div
      key={product._id}
      className="flex flex-row  col-span-4  justify-center align-middle cursor-pointer  items-center sm:flex-row py-8 px-8  pr-2 "
    >
      <div className=" my-20 w-[20rem]    sm:my-8">
        <Link href={`/product/${product._id}`}>
          <img
            className="rounded-md hover:opacity-80 hover:shadow-lg w-[40rem] transition duration-200 ease-out  cursor-pointer object-cover flex-shrink-0"
            src={product.img}
          />
        </Link>
      </div>
