    ...
    >
      <div className=" my-4   flex-shrink-0  sm:my-8">
        <Link href={`/product/${product._id}`}>
          <img
            loading="lazy"
            className="rounded-md flex-1 w-[10rem] h-[10rem] hover:opacity-80 hover:shadow-lg  transition duration-200 ease-out  cursor-pointer  object-cover  flex-shrink-0 "
            src={product.img}
          />
        </Link>
      </div>
      ...
      <div className="flex flex-col  pl-2">
        <h4 className="text-base max-w-xs sm:text-lg  font-semibold   sm:max-w-2xl line-clamp-1  mt-2 text-left sm:text-left">
          {product.title}{" "}
        </h4>
