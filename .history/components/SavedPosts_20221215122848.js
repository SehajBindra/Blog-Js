...
function SavedPosts({ product }) {
  ...
  useEffect(() => {
    ...
    onSnapshot(collection(db, "post", "saved", id), (snapshot) =>
  ...
  const savePost = async () => {
    const id = `${product._id}`;
    if (hasSaved) {
      await deleteDoc(doc(db, "post", "saved", id, session?.user?.name));
    } else {
      await setDoc(doc(db, "post", "saved", id, session?.user?.name), {
