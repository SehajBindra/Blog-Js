...
function ProductSection({ product }) {
  ...
  return (
    <div
      key={product._id}
      className="flex flex-col mx-auto col-span-2  justify-center align-middle cursor-pointer  items-center sm:flex-row py-8 px-8  pr-2 "
    >
      <div className=" my-20 h-60 w-72 md:h-80 md:w-[20rem]   sm:my-8">
        <Link href={`/product/${product._id}`}>
          <img
            className="rounded-md hover:opacity-80 hover:shadow-lg w-[40rem] h-[15rem] transition duration-200 ease-out  cursor-pointer object-cover flex-shrink-0"
            src={product.img}
          />
        </Link>
      </div>
