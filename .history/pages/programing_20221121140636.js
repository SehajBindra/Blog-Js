...
function programing({ product }) {
  return (
    <div>
      ...
      <div className="flex bg-black text-white h-screen flex-col overflow-x-scroll  scrollbar-hide sm:grid  sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3">
        {product.map((product) => (
          <div
            key={product._id}
          ...
          >
            <Link href={`/product/${product._id}`}>
              <div className=" cursor-pointer md:mt-2">
                <h2 className="line-clamp-2 max-w-xs sm:max-w-sm md:max-w-lg placeholder: font-semibold">
                  {" "}
                  {product.title}{" "}
                </h2>
