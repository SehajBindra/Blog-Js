...
export default function Home({ products, singleproduct }) {
  const currentUser = useSelector(Currentuser);
  const Currentpost = useSelector(currentPost);

  console.log(Currentpost.data.data._id);
  console.log(products, singleproduct);

  const handleDelete = async (id) => {
    try {
      await axios
        .delete(`api/products/632c2f75a4d64bd1839f01d5`, {
          data: {
...
export async function getStaticProps(params) {
  ...
  const allData = JSON.stringify(res2);
